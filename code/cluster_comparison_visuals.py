...
for user in d.keys():
    ...
    similar_x = list(chain.from_iterable(d[user][i][2] for i in range(1,len(list(d[user])))))
    similar_y = list(chain.from_iterable(d[user][i][1] for i in range(1,len(list(d[user])))))
